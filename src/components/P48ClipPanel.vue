...
const props = defineProps({
  ...
  member: { type: String, default: '谭思慧' },
...
async function startClip() {
  ...
  try {
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        ...
        const SOURCES = attempt < 2
          ...
          : [
              ...
              { url: p48.proxySegment(url), label: '后端' },
              { url, label: '直连' }
            ]
    ...
    if (completedCount === 0) {
      ...
      downloadBlob(new Blob([recordContent], { type: 'text/plain;charset=utf-8' }), `_clip_record_${props.member}_${formatBeijingDate()}.txt`)
